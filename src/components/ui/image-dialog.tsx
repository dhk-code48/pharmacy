...
interface DialogImageProps extends CldImageProps {}

function DialogImage({ className, style, ...props }: DialogImageProps) {
  const { uniqueId } = useDialog();

  return (
    <motion.div layoutId={`dialog-img-${uniqueId}`} style={style}>
      <CldImage {...props} className={cn(className)} />
    </motion.div>
  );
}
...
export {
  ...
  DialogContainer as ImageDialogContainer,
  DialogContent as ImageDialogContent,
  DialogClose as ImageDialogClose,
  DialogTitle as ImageDialogTitle,
  DialogSubtitle as ImageDialogSubtitle,
  DialogDescription as ImageDialogDescription,
  DialogImage,
};
