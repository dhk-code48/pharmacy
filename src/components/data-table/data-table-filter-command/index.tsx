...
import { getFieldOptions, getFieldValueByType, getFilterValue, getWordByCaretPosition, replaceInputByFieldType } from "./utils";
...
export function DataTableFilterCommand<TData, TSchema extends z.AnyZodObject>({
  ...
  const filterFields = useMemo(() => _filterFields?.filter((i) => !i.commandDisabled), [_filterFields]);
  const [inputValue, setInputValue] = useState<string>(serializeColumFilters(columnFilters, filterFields));
  ...
  useEffect(() => {
    ...
    const commandDisabledFilterKeys = currentDisabledFilters.reduce((prev, curr) => {
    ...
    <div>
      ...
      >
        ...
        <span className="h-11 w-full max-w-sm truncate py-3 text-left text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50 md:max-w-xl lg:max-w-4xl xl:max-w-5xl">
          {inputValue.trim() ? <span className="text-foreground">{inputValue}</span> : <span>Search data table...</span>}
        </span>
      ...
      <Command
        className={cn("overflow-visible rounded-lg border shadow-md [&>div]:border-none", open ? "visible" : "hidden")}
        filter={(value, search, keywords) => getFilterValue({ value, search, keywords, currentWord })}
        // loop
      >
        <CommandInput
          ref={inputRef}
        ...
        <div className="relative">
          <div className="absolute top-2 z-10 w-full overflow-hidden rounded-lg border border-accent-foreground/30 bg-popover text-popover-foreground shadow-md outline-none animate-in">
                    ...
                    return (
                      ...
                      >
                        ...
                        {facetedValue?.has(optionValue) ? (
                          <span className="ml-auto font-mono text-muted-foreground">{facetedValue?.get(optionValue)}</span>
                        ) : null}
                      </CommandItem>
                    );
                  });
            ...
            <div className="flex flex-wrap justify-between gap-3 border-t bg-accent/50 px-2 py-1.5 text-sm text-accent-foreground" cmdk-footer="">
              <div className="flex flex-wrap gap-3">
                <span>
                  Use <Kbd className="bg-background">↑</Kbd> <Kbd className="bg-background">↓</Kbd> to navigate
                </span>
                <span>
...
function CommandItemSuggestions<TData>({ field }: { field: DataTableFilterField<TData> }) {
  switch (field.type) {
    ...
    case "input": {
      return (
        <span className="ml-1 hidden truncate text-muted-foreground/80 group-aria-[selected=true]:block">[{`${String(field.value)}`} input]</span>
      );
    }
    default: {
      return null;
