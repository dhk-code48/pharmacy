...
export function DataTableFilterCheckobox<TData>({ table, value: _value, options, component }: DataTableFilterCheckboxProps<TData>) {
  ...
  const filterOptions = options.filter((option) => inputValue === "" || option.label.toLowerCase().includes(inputValue.toLowerCase()));
            ...
            >
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  const newValue = checked ? [...(filters || []), option.value] : filters?.filter((value) => option.value !== value);
              ...
              >
                {Component ? <Component {...option} /> : <span className="truncate font-normal">{option.label}</span>}
                <span className="ml-auto flex h-4 w-4 items-center justify-center font-mono text-xs">{facetedValue?.get(option.value)}</span>
                <button
                  type="button"
                  onClick={() => {
