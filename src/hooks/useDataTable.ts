...
export function useDataTable<TData, TValue>({
  ...
  // Search params
  const { page, per_page, sort } = schema.parse(Object.fromEntries(searchParams));
  ...
  React.useEffect(() => {
    ...
    for (const key of searchParams.keys()) {
      if (
        (searchableColumns.find((column) => column.id === key) && !debouncedSearchableColumnFilters.find((column) => column.id === key)) ??
        (filterableColumns.find((column) => column.id === key) && !filterableColumnFilters.find((column) => column.id === key))
      ) {
